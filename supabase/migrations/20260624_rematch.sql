...
-- Fecha: 2026-06-24
--
-- Al terminar una partida, cada jugador puede pedir revancha. El voto se
-- guarda en la partida (rematch_p1/p2) y se ve por realtime. Cuando ambos
-- quieren, request_rematch crea una NUEVA partida entre los mismos jugadores
-- (misma apuesta y objetivo, descontando la apuesta otra vez) y deja
-- rematch_game_id apuntando a ella para que ambos clientes naveguen allá.
-- Idempotente.
-- ============================================================

begin;

alter table public.games add column if not exists rematch_p1 boolean not null default false;
alter table public.games add column if not exists rematch_p2 boolean not null default false;
alter table public.games add column if not exists rematch_game_id uuid;
