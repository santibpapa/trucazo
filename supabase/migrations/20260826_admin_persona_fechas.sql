-- ============================================================
-- TRUCAZO — Arreglo: las fechas de la ficha salían con hora pegada
-- Fecha: 2026-08-26
--
-- EL PROBLEMA: entrar a la ficha de alguien QUE YA HABÍA JUGADO rompía la
-- página entera ("Application error"). A quien no había jugado nunca no le
-- pasaba, porque esa parte de la página ni se dibuja.
--
-- POR QUÉ: el gráfico de "cuándo jugó" arma la lista de los últimos 60 días
-- con generate_series(). Esa función, cuando se le dan fechas, devuelve
-- TIMESTAMP (fecha + hora), no fecha pelada. Así que en vez de mandar
-- "2026-06-26" mandaba "2026-06-26T00:00:00+00:00", y la página, que espera
-- solo la fecha, no sabía qué hacer con eso y se caía.
--
-- admin_stats (el panel principal) ya hacía la conversión bien; en admin_player
-- se pasó por alto. Acá se le pone el mismo ::date.
--
-- No cambia nada más de la función ni toca ningún dato. Idempotente.
-- ============================================================
