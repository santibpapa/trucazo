-- ============================================================
-- TRUCAZO — Bot: umbral de "quiero" por nivel de envido
-- Fecha: 2026-07-23 (correr DESPUÉS de las dos migraciones 20260720_bot_*:
-- el nombre y la fecha ordenan después, así la reconstrucción desde cero
-- las aplica en ese orden)
--
-- EL BUG (reportado: "el bot quiere la falta envido con pocos tantos; de 5
-- rivales de Santiago del Estero, a 3 les gané con falta"): el bot decidía
-- si quería el envido con UNA SOLA vara para los tres niveles — pedía el
-- mismo tanto para un envido simple (vale 2) que para una FALTA ENVIDO (que
-- se juega la partida). Esa vara era "tanto >= 20" en dificultad 7+, y una
-- mano cualquiera llega a 20 el ~60% de las veces: el bot quería la falta 6
-- de cada 10 veces con basura. Paradoja: como la vara baja al subir la
-- dificultad (27 - d, con piso 20), los rivales MÁS difíciles (Santiago:
-- dif 8-10) eran los más fáciles de robar con la falta.
--
-- EL ARREGLO: la vara ahora depende de QUÉ te responden:
--   * envido simple: igual que antes (greatest(20, 27 - d)).
--   * real envido:   un poco más (greatest(24, 29 - d)).
--   * falta envido:  tanto muy fuerte (greatest(29, 34 - d)); con eso el bot
--     quiere la falta ~13% de las veces con mano al azar, en vez del ~60%.
-- Además, ni el empujón de reputación ni el "acepto a ciegas" de los bots
-- tontos aplican a la falta: la falta se quiere solo con tanto de verdad.
--
-- El resto del cerebro queda EXACTAMENTE igual. Espejo en scripts/sim_bot.ts.
-- Idempotente.
-- ============================================================
